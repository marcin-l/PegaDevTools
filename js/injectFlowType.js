...
if(document.querySelector("div.gfw-breadcrumbs ul li")) {
    ...
    document.querySelector("a#PDTShowNodeIDs").addEventListener("click", () => {
        document.querySelectorAll("div#processFlow_g svg g g[id].typeAssignment, div#processFlow_g svg g g[id].typeUtility, div#processFlow_g svg g g[id].typeFork, div#processFlow_g svg g g[id].typeSubflow").forEach((e)=> {let gid = e.getAttribute('id'); console.log(e.querySelector("tspan").innerText); e.querySelector("tspan").innerHTML = gid; e.querySelectorAll("tspan:nth-of-type(n+2)").forEach(e => e.remove());});
    })
}
